...
( 
  ...
  select concept_id from CONCEPT where concept_id in (4329847)and invalid_reason is null
...
(
  ...
  select concept_id from CONCEPT where concept_id in (314666)and invalid_reason is null
...
CREATE TEMP TABLE qe_3 AS
  select E.person_id, E.start_date, E.end_date, row_number() OVER (PARTITION BY
    E.person_id ORDER BY E.start_date ASC) ordinal,
OP.observation_period_start_date as op_start_date,
OP.observation_period_end_date as op_end_date, cast(E.visit_occurrence_id as
    bigint) as visit_occurrence_id
  FROM qe_2 AS E
    JOIN observation_period OP on E.person_id = OP.person_id and E.start_date
    >=  OP.observation_period_start_date and E.start_date <=
...
op_start_date, op_end_date, cast(P.visit_occurrence_id as bigint) as
visit_occurrence_id
FROM qe_3 AS P;

CREATE TEMP TABLE vo_1 AS
  select vo.* 
  FROM VISIT_OCCURRENCE vo
  JOIN Codesets codesets on ((vo.visit_concept_id = codesets.concept_id and codesets.codeset_id = 1))
  WHERE person_id = 1780;

CREATE TEMP TABLE vo_2 AS
    select C.person_id, C.visit_occurrence_id as event_id,
    C.visit_start_date as start_date, C.visit_end_date as end_date,
    C.visit_concept_id as TARGET_CONCEPT_ID, C.visit_occurrence_id
    from vo_1 AS C;

CREATE TEMP TABLE vo_3 AS
SELECT 0 as index_id, p.person_id, p.event_id
FROM primary_events P
INNER JOIN vo_2 AS A
on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= (P.START_DATE + 0*INTERVAL'1 day') AND A.END_DATE >= (P.START_DATE + 0*INTERVAL'1 day') AND A.END_DATE <= P.OP_END_DATE
...
(
  ...
  INNER JOIN vo_3 AS CQ
  on E.person_id = CQ.person_id and E.event_id = CQ.event_id
...
DELETE FROM cohort where cohort_definition_id = 1770674;

INSERT INTO cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)

select 1770674 as cohort_definition_id, person_id, start_date, end_date 
