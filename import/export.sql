...
\copy (SELECT * FROM vocabulary WHERE vocabulary_id in ('Concept Class','Vocabulary','Domain','SNOMED','Relationship','Gender','Race','Ethnicity') ORDER BY vocabulary_id) TO 'vocabulary.csv' WITH CSV HEADER;
\copy (SELECT * FROM domain WHERE domain_id in ('Metadata','Condition','Gender','Race','Ethnicity') ORDER BY domain_id) TO 'domain.csv' WITH CSV HEADER;
\copy (SELECT * FROM concept_class WHERE concept_class_id in ('Vocabulary','Concept Class','Domain','Clinical Finding','Relationship','Model Comp','Gender','Race','Ethnicity') ORDER BY concept_class_id) TO 'concept_class.csv' WITH CSV HEADER;
