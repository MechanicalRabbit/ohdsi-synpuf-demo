...
\copy (select condition_occurrence_id, person_id, CASE condition_concept_id WHEN 444406 THEN 312327 ELSE condition_concept_id END AS condition_concept_id, condition_start_date, condition_end_date, condition_type_concept_id, stop_reason, provider_id, visit_occurrence_id, condition_source_value, condition_source_concept_id from condition_occurrence where person_id = 107680 and condition_concept_id in (select descendant_concept_id from concept_ancestor where ancestor_concept_id = 4329847) order by condition_occurrence_id) TO 'condition_occurrence.csv'  WITH CSV HEADER;
