...
\copy (SELECT * FROM vocabulary WHERE vocabulary_id in ('Concept Class','Vocabulary','Domain','SNOMED','Relationship','Gender','Race','Ethnicity','Visit','None','Visit Type','Obs Period Type','CMS Place of Service','Cohort') ORDER BY vocabulary_id) TO 'vocabulary.csv' WITH CSV HEADER;
...
\copy (SELECT * FROM concept_class WHERE concept_class_id in ('Vocabulary','Concept Class','Domain','Clinical Finding','Relationship','Model Comp','Gender','Race','Ethnicity','Visit','Undefined','Visit Type','Obs Period Type','Cohort') ORDER BY concept_class_id) TO 'concept_class.csv' WITH CSV HEADER;
...
\copy (select * from condition_occurrence where person_id = 107680 and condition_concept_id in (select descendant_concept_id from concept_ancestor where ancestor_concept_id = 4329847) order by condition_occurrence_id) TO 'condition_occurrence.csv'  WITH CSV HEADER;
